...
<section id="contents">
	...
	<div class="title_box">
		<h3 class="title">페이지 카테고리 관리</h3>
		<div class="location">
			<i class="xi-home"></i> <span>컨텐츠 관리</span> <strong>페이지 카테고리 관리</strong>
		</dl>
	</div>
		...
		<div class="tb_list cate-list" style="width:18%">
			...
			<table>
				...
				If Len(s_cate)>=2 Then
					...
					Sql = "SELECT * FROM tbl_page_cate WHERE c_depth='2' AND c_code LIKE '" & Left(s_cate, 2) & "%' ORDER BY c_lev ASC"
		...
		<div class="tb_list cate-list" style="width:18%">
			...
			<table>
				...
				If Len(s_cate)>=4 Then
					...
					Sql = "SELECT * FROM tbl_page_cate WHERE c_depth='3' AND c_code LIKE '" & Left(s_cate, 4) & "%' ORDER BY c_lev ASC"
		...
		<div class="tb_list" style="width:40%; float:right; height:500px">
			...
			If mode="reg" Or mode="mod" Then
				...
				If mode="reg" Then
					...
					'언어정보 취득
					Sql = "SELECT * FROM tbl_page_cate WHERE c_code='" & s_cate & "' "
					RsP.Open Sql, Dbcon, 1
					If RsP.EOf=False Then
						c_lang = RsP("c_lang")
					End If
