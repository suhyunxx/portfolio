...
If k_prevnext=True Then
	...
	If c_notice=False Then
		...
		Sql = "SELECT TOP 1 idx, notice, title, comment_count, regdate, lock, mem_id FROM tbl_board_data WHERE num<'" & c_num & "' AND d_num=0 AND board_idx='" & bid & "' AND notice='0' ORDER BY num DESC"
		Rs.open Sql, Dbcon, 1
		...
		Sql = "SELECT TOP 1 idx, notice, title, comment_count, regdate, lock, mem_id FROM tbl_board_data WHERE num>'" & c_num & "' AND d_num=0 AND board_idx='" & bid & "' AND notice='0' ORDER BY num ASC"
		Rs.open Sql, Dbcon, 1
		If Rs.EOF=False Then
			bdn_idx = Rs(0)
			bdn_notice = Rs(1)
	...
	<div class="ntb-tb-view-reply">
		...
		<table width="<%=T_WIDTH%>" cellpadding="0" cellspacing="0" class="ntb-tb-view">
			...
			<tr> 
				<th>Previous post</th>
				<td class="left font_gray">No data</td>
			</tr>
			<%End If%>	
			<!--다음글 시작-->
			<%if bdn_idx<>"" Then%>
