...
<table class="ntb-tb-view" style="width:<%=T_WIDTH%>" cellpadding="0" cellspacing="0">
	...
	<tr>
		...
		<%
		'구글 맵 연동 표시
		If c_addtext3<>"" And c_addtext4<>"" Then%>
			<article class="map" id="map" style="height:400px"></article>
			<script>
				function initMap() {
				  var map = new google.maps.Map(document.getElementById('map'), {
					zoom: 16,
					center: {lat: <%=c_addtext4%>, lng: <%=c_addtext3%>}
				  });

				  var image = '/site/bbs/skin/resp_agent_map2/images/point_b.png';
				  var beachMarker = new google.maps.Marker({
					position: {lat: <%=c_addtext4%>, lng:<%=c_addtext3%> },
					map: map,
					icon: image
				  });
				}
			</script>
			<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDl5nNv45o4Xnn7OOWgGlFd1aHeOhGbUaA&callback=initMap"></script>
		<%End If%>
	...
	<%For fc=1 To k_upload-1
		If c_file(fc)<>"" Then%>
		<tr>
			<td colspan="2" class="left"><img src="/site/images/board/file.gif" class="vmiddle" alt="file<%=fc%>" /> File #<%=fc%>
