 ...
 ******************************************************************************/
case "type1":
  $cfg['btn_first'] = '<i class="bi bi-chevron-double-left"></i>';
  $cfg['btn_prev']  = '<i class="bi bi-chevron-left"></i> ';
  $cfg['btn_next']  = ' <i class="bi bi-chevron-right"></i>';
  $cfg['btn_last']  = '<i class="bi bi-chevron-double-right"></i>';


echo '<div class="__paging">';

  /*-------------------------------------------------------------------------------------------------
  ▶ 맨 처음으로 */
  if($cfg['btn_first'] != ""){
    if ( $req['pagenumber'] > $req['page_block'] ) echo " <a href='javascript:$func(1);' class=\"arr first\">".$cfg['btn_first']."</a>";
    else echo "<a href=\"#\" class=\"arr first\">".$cfg['btn_first']."</a>";
  }

  /*-------------------------------------------------------------------------------------------------
  ▶ 이전 10 */
  if ( $totalpage > $req['page_block'] && $req['pagenumber'] > $req['page_block'] ) echo " <a href='javascript:$func(".$prevpage.");' class=\"arr prev\">".$cfg['btn_prev']."</a>";
  else echo "<a href=\"#\" class=\"arr prev\">".$cfg['btn_prev']."</a>";

  /*-------------------------------------------------------------------------------------------------
  ▶ 페이징 리스트 */
  if ( $totalpage <= 1 ) {
      echo "<a  href='#none' class=\"num active\">1</span>";
  } else {
    // 페이지 링크 번호 나열
    if($startpage < 1 )$startpage = 1;
    for ( $i = $startpage ; $i <= ($startpage + ($req['page_block'] - 1) ) ; $i++ ) {

      if( $req['pagenumber'] != $i ) echo "<a href='javascript:$func(".$i.")' class=\"num\">".$i."</a>";
      else	echo "<a href='#none' class=\"num active\">$i</a>";
  ...
  /*-------------------------------------------------------------------------------------------------
  ▶ 다음 10 */
  if ( $startpage + $req['page_block'] - 1 < $totalpage) echo " <a href='javascript:$func(".$nextpage.")' class=\"arr next\">".$cfg['btn_next']."</a>";
  else echo "<a href=\"#\" class=\"arr next\">".$cfg['btn_next']."</a>";

  /*-------------------------------------------------------------------------------------------------
  ▶ 맨 끝으로 */
  if($cfg['btn_last'] != ""){
    if ( $req['pagenumber'] < intval($laststartpage) ) echo " <a href='javascript:$func(".$totalpage.");' class=\"arr last\">".$cfg['btn_last']."</a>";
    else 	echo "<a href=\"#\" class=\"arr last\">".$cfg['btn_last']."</a>";
  }
